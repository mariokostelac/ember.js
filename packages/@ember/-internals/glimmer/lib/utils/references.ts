...
export class RootReference<T extends object> extends ConstReference<T>
  ...
  get(propertyKey: string): VersionedPathReference<Opaque> {
    ...
    if (ref === undefined) {
      ref = this.children[propertyKey] = RootPropertyReference.create(this.inner, propertyKey);
    }
...
export class RootPropertyReference extends PropertyReference
  ...
  static create(parentValue: object, propertyKey: string) {
    if (isProxy(parentValue)) {
      return new ProxyPropertyReference(parentValue, propertyKey);
    } else {
  ...
  compute(): Opaque {
    ...
    let ret = this.read(parentValue, propertyKey);
  ...
  protected read(value: object, key: string): Opaque {
    return value[key];
  }
}

export class ProxyPropertyReference extends RootPropertyReference {
  protected read(value: object, key: string): Opaque {
...
function valueKeyToRef(value: Opaque, key: string): VersionedPathReference<Opaque> {
  if (isObject(value)) {
    // root of interop with ember objects
    return RootPropertyReference.create(value, key);
  } else if (isFunction(value)) {
    // ember doesn't do observing with functions
